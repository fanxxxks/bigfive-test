...
export default function MoneyResultsReport() {
  ...
  useEffect(() => {
    const s = sessionStorage.getItem('money_result');
    if (s) { try { const p = JSON.parse(s); if (p?.styles?.length) setR(p); } catch {} }
  }, []);
  if (!r) return (<div className="text-center py-20"><p className="text-gray-400 text-lg">未找到测评结果。</p><a href={`${import.meta.env.BASE_URL}money`} className="btn-primary inline-block mt-4">去测评</a></div>);
  const { primary, styles } = r;
  const { chartRef, downloadPoster } = usePosterDownload('金钱观念雷达图.png');
  return (<div className="space-y-8">
    <div className="card text-center"><div className="text-5xl mb-4">{primary.emoji}</div><h1 className="text-2xl font-bold text-gray-900 mb-2">您的金钱观念报告</h1><p className="text-xl font-bold mb-2" style={{ color: primary.color }}>{primary.name}</p><p className="text-lg text-gray-500 italic mb-3">"{primary.motto}"</p><p className="text-gray-600 max-w-xl mx-auto leading-relaxed">{primary.description}</p><p className="text-xs text-gray-400 mt-3">报告生成时间：{new Date(r.timestamp).toLocaleString('zh-CN')}</p></div>
    <div className="card"><h2 className="text-lg font-bold text-gray-800 mb-4">5种金钱人格完整排序</h2><div style={{ height: 300 }}><ReactECharts ref={chartRef} option={barOpt(r)} style={{ height: '100%' }} /></div></div>
      ...
      <div className="flex items-center justify-center gap-4 flex-wrap">
        ...
        <button
          onClick={() => downloadPoster({
            title: '金钱观念图谱',
            subtitle: `${primary.emoji} ${primary.name} · "${primary.motto}"`,
            emoji: primary.emoji,
